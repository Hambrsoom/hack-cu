...
class Heatmap extends Component<Props> {
  ...
  onLocationChanged = async (location: LocationObject) => {
    ...
    if (lastLocationUpdate) {
      ...
      let locationNeedsToBePersistedAgain: boolean =
        currentTime.getTime() - lastPersistedTime.getTime() <
        FIVE_MINUTES_IN_MS;
  ...
  showMap(show: any) {
    if (show) {
      return (
        ...
        >
          <HeatmapRN points={this.state.heatMapPoints} radius={50} />
        </MapView>
      );
    }
